...
import HeroAromaTrail from "@/components/rasa/HeroAromaTrail";
...
export default function Hero() {
  ...
  return (
    <section
      className="relative min-h-screen flex items-center pt-[120px] pb-[70px] overflow-hidden"
      style={{
        background:
    ...
    >
      {/* Full-bleed feast atmosphere — stronger on desktop, soft on mobile */}
      <div
        aria-hidden="true"
      ...
      <div
        ...
        style={{
          background:
            "linear-gradient(105deg, rgba(28,16,27,.96) 0%, rgba(28,16,27,.88) 38%, rgba(34,20,33,.55) 68%, rgba(34,20,33,.72) 100%), linear-gradient(180deg, rgba(28,16,27,.35) 0%, transparent 40%, rgba(28,16,27,.75) 100%)",
        }}
      />
      ...
      <div
        aria-hidden="true"
        className="absolute right-0 top-1/2 -translate-y-1/2 font-deva pointer-events-none select-none"
        style={{
          fontSize: "clamp(20rem,46vw,50rem)",
          lineHeight: 1,
          color: "transparent",
          WebkitTextStroke: "1.5px rgba(226,182,88,.16)",
        }}
      >
        रस
      </div>
      ...
      <div className="relative z-10 max-w-[1220px] mx-auto px-5 sm:px-7 max-w-[860px]">
        ...
        <h1
          className="font-display font-normal leading-[1.06] tracking-[-0.025em] mb-4"
        ...
        <p
          className="text-[clamp(1.02rem,1.7vw,1.24rem)] font-light max-w-[600px] mb-9"
        ...
        >
          ...
          <span className="font-medium" style={{ color: "var(--ivory)" }}>highly trained chefs</span> — then served with
          grace, without the frills, at honestly fair rates.
        </p>

        <div className="flex flex-wrap gap-4">
          <button
            onClick={() => scrollTo("packages")}
            className="glossy-btn-gold inline-flex items-center gap-2 px-[30px] py-[15px] rounded-[44px] text-[0.92rem] font-semibold tracking-[0.03em]"
          >
          ...
          <button
            ...
            className="glossy-btn-ghost px-[30px] py-[15px] rounded-[44px] text-[0.92rem] font-semibold tracking-[0.03em]"
          >
            Our Story
          </button>
        </div>
        ...
        <div className="flex flex-wrap gap-0 mt-16 pt-[22px] border-t" style={{ borderColor: "var(--paper-line)" }}>
          {TASTE_DOTS.map((t) => (
            <div key={t.en} className="flex-1 min-w-[110px] py-1.5 pr-4">
              ...
              <span
                className="block text-[0.66rem] tracking-[0.24em] uppercase mt-[3px] pl-[18px]"
                style={{ color: "rgba(246,239,224,.62)" }}
              >
                {t.sa}
              </span>
