...
/** Official WhatsApp mark — distinct from in-site ChatWidget on the right */
...
/** Bottom-left: WhatsApp + Call. Site chat stays on the right (ChatWidget). */
export default function FloatingButtons() {
  const telUrl = `tel:${CONFIG.phoneDial}`;
  const waText = encodeURIComponent(
    `Namaste! I would like to enquire about catering with Rasa by Narayanam.`
  );
  ...
  return (
    <div className="fixed bottom-6 left-6 z-[95] flex flex-col gap-3">
      <a
        ...
        style={{
          background: "linear-gradient(180deg,#2a9970,#1f7a5c)",
          color: "#fff",
          boxShadow: "0 8px 22px -6px rgba(31,122,92,0.65)",
        }}
        aria-label="WhatsApp"
        title="WhatsApp"
      >
