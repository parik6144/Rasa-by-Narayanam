    ...
    <section className="py-[104px]" id="packages">
      <div className="max-w-[1220px] mx-auto px-5 sm:px-7">
        <div className="mb-14 max-w-[720px]">
          ...
          <h2 className="font-display mb-4" style={{ fontSize: "clamp(2.1rem,4.4vw,3.5rem)", color: "var(--ivory)" }}>
            Five tables, one standard.
          </h2>
          <p className="text-[1.06rem] font-light" style={{ color: "rgba(246,239,224,.62)" }}>
            Each package sets a per-guest price and a generous set of choices. Tap one to open its menu builder — tick
            your dishes course by course, add custom dishes if you want, then send your complete menu to us on WhatsApp
            or email.
          </p>
        </div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-[22px]">
          {packages.map((p, i) => {
            ...
            const img = packageImage(p.id, i);
            return (
              <div
                key={p.id}
                className="glossy-card rounded-md flex flex-col relative overflow-hidden group"
                style={p.featured ? { borderColor: "rgba(226,182,88,.6)" } : undefined}
              >
                <div className="relative h-[168px] overflow-hidden" style={{ borderBottom: "1px solid var(--paper-line)" }}>
                  <img
                    src={img}
                    alt={`${p.name} catering`}
                  ...
                  <div
                    ...
                    style={{
                      background:
                        "linear-gradient(180deg, rgba(34,20,33,.12) 0%, rgba(34,20,33,.45) 50%, rgba(34,20,33,.92) 100%)",
                    }}
                  />
                  {p.featured && (
                    <span
                      className="absolute top-3 right-3 text-[0.6rem] tracking-[0.2em] uppercase px-[10px] py-1 rounded-full font-bold glossy-btn-gold z-10"
                      style={{ color: "#231318" }}
                    >
                      Most Loved
                    </span>
                ...
                <div className="p-[26px_28px_30px] flex flex-col flex-1">
                  <div className="font-display text-[1.62rem] mb-1" style={{ color: "var(--ivory)" }}>
                    {p.name}
                  </div>
                  <div className="font-display italic text-[1rem] mb-3" style={{ color: "var(--gold-bright)" }}>
                    {p.tagline}
                  </div>
                  ...
                  <div className="font-display text-[2.7rem] my-[18px_0_2px]" style={{ color: "var(--ivory)" }}>
                    ₹{p.price}
                    <span
                      className="block text-[0.8rem] tracking-[0.1em] font-normal mt-[-2px]"
        ...
        <p className="text-center mt-10 text-[0.92rem] font-light" style={{ color: "rgba(246,239,224,.62)" }}>
          ...
          <a href="#addons" className="font-semibold" style={{ color: "var(--gold-bright)" }}>
            Add-ons catalogue
          </a>{" "}
          can be served on top, on a per-guest or per-event basis.
        </p>
      </div>
    </section>
  );
